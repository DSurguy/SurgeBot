...
var diceGame = function(bot, from, to, params){
		...
		case 3:
			...
			//check if this player has enough currency and then start the game
				...
				if( checkCurrency(from, args.bid) ){
					...
					//not enough cash
					bot.client.say(to, from+' doesn\'t have enough '+_currency.name+' to join the game!');
				}
			}
		break;
		case 1:
			...
			if( !args.end && !args.play ){
			...
			else if( (args.end || args.play) && _diceGame.state == 0 ){
				//game not started
				bot.client.say(to, 'game no start');
			}
			else if( args.end && _diceGame.host !== from ){
				//not correct host
				bot.client.say(to, "INCORRECTO HOSTO");
			}
			else if( args.end ){
				//correct host, end the game and payout
				var results = _diceGame.players.reduce(function (resultObj, player){
				...
				bot.client.say(to, '\x0314'+results.loser+' sends '+(results.winnerRoll-results.loserRoll)+' '+_currency.name+' to '+results.winner+'.');
			}
			else if( args.play ){
				//search the players list for this user
				var userFound = _diceGame.players.reduce(function (found, player){
				...
				if( userFound ){
					//user already rolled
					bot.client.say(from, 'You have already joined the game in progress with a roll of '+_diceGame.players.reduce(function (roll, player){
					...
					var playerRoll = (Math.floor(Math.random() * (_diceGame.bid-1))+1);
					_diceGame.players.push({
						player: from,
						roll: playerRoll
					});
					bot.client.say(to, from+' joined the game and rolled \x0303 '+playerRoll+' \x0314(out of '+_diceGame.bid+')');
				}
			}
		break;
		default:
...
var checkCurrency = function(player, amount){
	var bot = this;
	if( auth.authedUsers[player] && auth.authedUsers[player].currency < amount ){
		return false;
	}
	return true;
};
